...
function make_tensor_array(func: Function, d: Array<number>): Array<any> {
  if (d.length > 0) {
    return Array(d[0])
      .fill(null)
      .map(() => make_tensor_array(func, d.slice(1)));
  } else {
    return func();
  }
}
...
function validate_dimensions(t: Tensor, d: Array<number>): boolean {
  if (t.data instanceof Array) {
    const valid_children: boolean = t.data.every(child =>
      validate_dimensions(child, d.slice(1))
    );
...
function validate_copy(a: Tensor, b: Tensor): boolean {
  if (a.data instanceof Array && b.data instanceof Array) {
    const valid_children_a: boolean = a.data.every((child, index) =>
      validate_copy(child, b.data[index])
    );
    const valid_children_b: boolean = b.data.every((child, index) =>
      validate_copy(child, a.data[index])
    );
